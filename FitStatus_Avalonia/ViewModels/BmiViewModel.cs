...
namespace FitStatus_Avalonia.ViewModels
{
    public class BmiViewModel
    {
                ...
                double.TryParse(HeightTextBox.Text, out height))
            {
                ...
                if (BmiResultTextBlock != null)
                {
                    ...
                    else if (bmi >= 18.5 && bmi < 25)
                    {
                        ...
                        AddBmiRecord($"Twoja waga jest prawidłowa.\nTwoje BMI wynosi: {bmi.ToString("0.00")}.");
        ...
        private void AddBmiRecord(string bmi)
        {
            string info = bmi;
            DateTime time = DateTime.Now;

            Bmi bmiRecord = new Bmi { Info = info, Time = time };

            DataAccess.AddBmiRecord(bmiRecord);
        }
